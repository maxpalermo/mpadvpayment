...
require_once dirname(__FILE__) . DIRECTORY_SEPARATOR
        ...
        . 'mpadvpayment' . DIRECTORY_SEPARATOR
...
abstract class PaymentModuleCore extends Module
{
        ...
        $secure_key = false, Shop $shop = null)
    {
        ...
        classMpLogger::addEvidencedMsg('START VALIDATE ORDER FROM CART');
        //classMpLogger::add('Cart ' . $this->context->cart->id . ' details before validating: ' . print_r($this->context->cart->getProducts(), 1));
        
        if (!isset($this->context)) {
            $this->context = Context::getContext();
        }
        $this->context->cart = new Cart((int)$id_cart);
        //classMpLogger::add('Cart ' . $id_cart . ' details after set context: ' . print_r($this->context->cart->getProducts(), 1));
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            classMpLogger::add("Cart total paid: " . $cart_total_paid);
            classMpLogger::add("Cart Details count: " . count($this->context->cart->getProducts()));
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    //classMpLogger::add('Product list: ' . print_r($order->product_list, 1));
                    ...
                    classMpLogger::add('Total paid: ' . $order->total_paid);
                    ...
                    //classMpLogger::add('Cart details: ' . print_r($this->context->cart->getProducts(), 1));
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    $fees = new ClassPaymentFee();
                    $summary = classSession::getSessionSummary();
                    if ($summary->payment_type == classCart::CASH) {
                        $summary_cart = $summary->cash->cart;
                    } elseif ($summary->payment_type == classCart::BANKWIRE) {
                        $summary_cart = $summary->bankwire->cart;
                    } elseif ($summary->payment_type == classCart::PAYPAL) {
                        $summary_cart = $summary->paypal->cart;
                    }
                    
                    $fees->create((int)$order->id, $summary->payment_type, $summary_cart->getFee(), $summary_cart->getFeeTaxRate());
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        '{total_fees}' => Tools::displayPrice($fees->getFee(), $this->context->currency, false),
                        '{total_document}' => Tools::displayPrice($fees->getTotal_document(), $this->context->currency, false),
